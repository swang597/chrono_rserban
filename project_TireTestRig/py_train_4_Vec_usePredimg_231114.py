...
parser.add_argument('--NN_idx', type=int, default=1, help='an NN_idx integer: 0-Img, 1-Vec, 2-(Img,Vec). Default is 0.')
...
epoch_start = 0
nepoch = 5000   # number of epochs to run
...
fn_dataset_test = f"/home/swang597/Documents/Research/chrono_fork_radu/project_TireTestRig/build/DEMO_OUTPUT/Dataset_4_ML_test/"

# # dataset_NN1_train = MyDataset_4wheel(read_multiple_dataset(fn_4wheel, filename))
# if wheel_cur == 10:
#     wheel_list = [0,1]
#     state_dict_modelImg = state_dict_modelImg_wheel10
# else:
#     wheel_list = [2,3]
#     state_dict_modelImg = state_dict_modelImg_wheel23

dataset_NN1_train = MyDataset_4wheel(read_multiple_dataset(fn_4wheel,'Iin_NN1_ts_train.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Vec_NN2_ts_train.pt',wheel_list), read_multiple_dataset(fn_4wheel,'F_NN1_ts_train.pt',wheel_list))
dataset_NN2_train = MyDataset_4wheel(read_multiple_dataset(fn_4wheel,'Iin_NN2_ts_train.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Vec_NN2_ts_train.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Iout_NN2_ts_train.pt',wheel_list))
dataset_NN1_test = MyDataset_4wheel(read_multiple_dataset(fn_4wheel,'Iin_NN1_ts_test.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Vec_NN2_ts_test.pt',wheel_list), read_multiple_dataset(fn_4wheel,'F_NN1_ts_test.pt',wheel_list))
dataset_NN2_test = MyDataset_4wheel(read_multiple_dataset(fn_4wheel,'Iin_NN2_ts_test.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Vec_NN2_ts_test.pt',wheel_list), read_multiple_dataset(fn_4wheel,'Iout_NN2_ts_test.pt',wheel_list))
print(f"len of dataset_wheels {wheel_list}:{len(dataset_NN1_train),len(dataset_NN1_test)}")
...
folder_model_4wheels = f"Model/Model_2stepNN_iDT{nDT}_img{img_rowsize}by{img_colsize}_usePredI{flag_usePredI}_{today_str}/"
if not os.path.exists(folder_model_4wheels):
    os.makedirs(folder_model_4wheels)
    print(f"mkdir {folder_model_4wheels}")

folder_model_wheel = folder_model_4wheels+f"wheel{wheel_cur}/"
...
for iepoch in range(epoch_start, epoch_start+nepoch):
    ...
    if iepoch - best_epoch > 60 and iepoch - epoch_reduced > 60:
        needReduceLr = 1
    # stop training if no improvement after 200 epoch
    if iepoch - best_epoch > 100:# and iepoch - epoch_start > 300:
