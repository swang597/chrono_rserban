...
int main(int argc, char *argv[]) {
    ...
    if (argc != 10) {
        std::cout << "Wrong argv.\nUsage: " << argv[0] 
        << " <dt> <terrain_grid> <time_tot> <dt_HM> <terrain_initX> <terrain_initH> <normal_load_wheel1><normal_load_wheel2><fixed_Vx>" << std::endl;
        return 1;
    }
    ...
    double normal_load_wheel1 = std::stod(argv[7]); // 1000;
    double normal_load_wheel2 = std::stod(argv[8]); // 1000;
    double fixed_Vx = std::stod(argv[9]); // 0.2;

    double dx_2wheels = -1.0, dy_2wheels = 0.0, dz_2wheels = 0.0;

    double time_delay = sqrt(2 * std::abs(terrain_initH) / 9.81); //sqrt(2*abs(H)/9.81) + 0.5
    time_tot += time_delay;
    ...
    bool flag_heightmap_save_1frame = true; //false;  //
    ...
    int save_vedio_fps = int(num_steps/10.0);
    int idx_vedio = 0;
    ...
    // ChTireTestRig2Wheels rig(wheel, wheel2, tire, tire2, sys);
...
// Create the Blender exporter
    ...
    for (int istep = 0; istep < num_steps; istep++) {
        ...
        // }
        auto& loc = rig.GetPos();
        auto& loc2 = rig.GetPos2();
        if(flag_vis){
            // vis->UpdateCamera(loc + ChVector<>(1.0, 2.5, 0.5), loc + ChVector<>(0, 0.25, -0.25));
            vis->UpdateCamera((loc + loc2)*0.5 + ChVector<>(-3.0, 3, 2), (loc + loc2)*0.5 + ChVector<>(-1, 0, 0));
            vis->BeginScene();
            vis->Render();
            vis->EndScene();
        }
        ...
        // Save heightmap
        if((flag_heightmap_save && istep % ndt_HM == 0) || (flag_heightmap_save_1frame)){
            if(time >= time_delay && wheel_state_wheel2.pos[0] > -0.22 ){
                
                flag_heightmap_save_1frame = false;
