...
// Shu-----------------------------------------------------------------------------
torch::Tensor applyWheelSinkageUpdateHM(torch::Tensor& heightMap, torch::Tensor wheelCenter, 
                                torch::Tensor gridOrigin, float radius, float width, float gridSize) {
    // Extract wheel center coordinates
    float cx = wheelCenter[0].item<float>();
    float cy = wheelCenter[1].item<float>();
    ...
    auto diff = radius * radius - distance_x.pow(2);
    // std::cout << "diff.min()=" << diff.min().item<float>() << ", diff.max()=" << diff.max().item<float>() << std::endl;
    auto diff_clamped = torch::clamp(radius * radius - distance_x.pow(2), 0);
    auto wheel_surface_height = cz - torch::sqrt(diff_clamped);

    // Calculate wheel surface height
    // auto wheel_surface_height = cz - torch::sqrt(radius * radius - distance_x.pow(2));
    // std::cout << "wheel_surface_height.size()=" << wheel_surface_height.sizes() << std::endl;
    // std::cout << "heightMap.index({0, 0}).size()=" << heightMap.index({0, 0}).sizes() << std::endl;
    
    // Compute the element-wise minimum between wheel_surface_height and the heightMap at index {0, 0}
    auto min_heights = torch::min(wheel_surface_height, heightMap.index({0, 0}));
    min_heights = min_heights.unsqueeze(0).unsqueeze(0);
    // std::cout << "min_heights.size()=" << min_heights.sizes() << ",min_heights[0][0][0][0]="<<min_heights[0][0][0][0]<< std::endl;
    return min_heights;
}
// Shu -----------------------------------------------------------------------------
class TerrainForceLoader : public ChLoadContainer {
  public:
    ...
    std::shared_ptr<hmmwv::HMMWV_Wheel> wheel1, std::shared_ptr<hmmwv::HMMWV_Wheel> wheel2, 
        ...
        m_GHM_0(GHM_0), m_terrain_initLoc(terrain_initLoc), 
    ...
    virtual void Setup() override {
        ...
        // std::cout << "*****ML.cpp: Line 315" << std::endl;
        ...
        // std::cout << "*****ML.cpp: Line 340" << std::endl;
        ...
        HM_wheel2_org = m_GHM_0.get_local_heightmap_by_origin(gridOrigin_wheel2, m_HM_nx+1, m_HM_ny+1);
        // std::cout << "*****ML.cpp: Line 347" << std::endl;
        // std::cout << "HM_wheel1_org[0][0][0][0]=" << HM_wheel1_org[0][0][0][0] << std::endl;
        // std::cout << "HM_wheel1_org.size=" << HM_wheel1_org.sizes() << std::endl;
        // Update HM by Geometry compute applyWheelSinkage
        HM_wheel1_cur = applyWheelSinkageUpdateHM(HM_wheel1_org, position_ts_wheel1, gridOrigin_wheel1, 
                                m_wheel_radius, m_wheel_width, m_heightmap_grid);
        HM_wheel2_cur = applyWheelSinkageUpdateHM(HM_wheel2_org, position_ts_wheel2, gridOrigin_wheel2,
                                m_wheel_radius, m_wheel_width, m_heightmap_grid);
        // std::cout << "HM_wheel1_cur[0][0][0][0]=" << HM_wheel1_cur[0][0][0][0] << std::endl;
        // std::cout << "HM_wheel1_cur.min=" << HM_wheel1_cur.min().item<float>() << ",HM_wheel1_cur.max=" << HM_wheel1_cur.max().item<float>() << std::endl;
        // std::cout << "*****ML.cpp: Line 355" << std::endl;
        ...
        // std::cout << "*****ML.cpp: Line 361" << std::endl;
        // std::cout << "w00_wheel1="<<w00_wheel1 <<",w01_wheel=" << w01_wheel1 << ", w10_wheel=" << w10_wheel1 << ", w11_wheel=" << w11_wheel1 << std::endl;
        // std::cout << "HM_wheel1_cur.size=" << HM_wheel1_cur.sizes() << std::endl;
        // std::cout << "HM_wheel1_cur_00.size=" << HM_wheel1_cur_00.sizes() << std::endl;
        // std::cout <<"HM_wheel1_cur_00.index.size" <<HM_wheel1_cur_00.index({torch::indexing::Slice(0, m_HM_nx), torch::indexing::Slice(0, m_HM_ny)}).sizes()<< std::endl;
        ...
        // std::cout << "*****ML.cpp: Line 366" << std::endl;                    
        ...
        // std::cout << "*****ML.cpp: Line 370" << std::endl;
        ...
        // std::cout << "*****ML.cpp: Line 382" << std::endl;
        // std::cout << "HM_wheel1_cur_00[0][0]=" << HM_wheel1_cur_00[0][0] << std::endl;
        m_GHM_0.update_heightmap_by_origin(gridOrigin_wheel1, HM_wheel1_cur);
        m_GHM_0.update_heightmap_by_origin(gridOrigin_wheel2, HM_wheel2_cur);
            
        auto start_F = std::chrono::high_resolution_clock::now();
        Vec_ts_wheel1 = torch::tensor({m_wheel1_state.pos[0], m_wheel1_state.pos[1], m_wheel1_state.pos[2],
                        m_wheel1_state.lin_vel[0], m_wheel1_state.lin_vel[1], m_wheel1_state.lin_vel[2],
                        m_wheel1_state.ang_vel[0], m_wheel1_state.ang_vel[1], m_wheel1_state.ang_vel[2]}).to(torch::kFloat32); // Vx, Vz, Wy
        Vec_ts_wheel1 = Vec_ts_wheel1.unsqueeze(0);
        Vec_ts_wheel2 = torch::tensor({m_wheel2_state.pos[0], m_wheel2_state.pos[1], m_wheel2_state.pos[2],
                        m_wheel2_state.lin_vel[0], m_wheel2_state.lin_vel[1], m_wheel2_state.lin_vel[2],
                        m_wheel2_state.ang_vel[0], m_wheel2_state.ang_vel[1], m_wheel2_state.ang_vel[2]}).to(torch::kFloat32); // Vx, Vz, Wy
        Vec_ts_wheel2 = Vec_ts_wheel2.unsqueeze(0);
            
        // std::cout << "*****ML.cpp: Line 396" << std::endl;
        ...
        if (remainder < 1e-6 || (m_dt_dump - remainder) < 1e-6) {
            // std::cout << "*****ML.cpp: Line 407" << std::endl;
            ...
            // std::cout << "*****ML.cpp: Line 412" << std::endl;
            ...
            // std::cout << "*****ML.cpp: Line 415" << std::endl;
            ...
            // std::cout << "*****ML.cpp: Line 418" << std::endl;
        }
        // std::cout << "*****ML.cpp: Line 420" << std::endl;
        // Invoke base class method
        ChLoadContainer::Update(ChTime, true);
        // std::cout << "*****ML.cpp: Line 421" << std::endl;
    }
...
int main(int argc, char *argv[]) {
    ...
    auto GHM_0_H = GHM_0.get_global_heightmap();
    std::cout << "*****GHM_0_H[0][0][0][0] = " << GHM_0_H[0][0][0][0] << std::endl;
