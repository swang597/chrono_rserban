...
void ChTireTestRig2Wheels::CreateMechanism(Mode mode) {
    ...
    m_carrier_body->SetMass(mass*0.05);
    m_carrier_body->SetInertiaXX(inertia*0.05);
    //Shu.
    {
    ...
    m_chassis_body->SetMass(mass*0.05);
    m_chassis_body->SetInertiaXX(inertia*0.05);
    //Shu.
    {
        auto mat = chrono_types::make_shared<ChVisualMaterial>();
    ...
    m_slip_body->SetMass(mass*0.05);
    m_slip_body->SetInertiaXX(inertia*0.05);
    //Shu.
    {
        auto mat = chrono_types::make_shared<ChVisualMaterial>();
        mat->SetDiffuseColor({0.2f, 0.8f, 0.2f});
