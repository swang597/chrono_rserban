...
namespace vehicle
{
    ...
    bool ChTire::disc_terrain_contact_4pt(const ChTerrain& terrain, // [in] reference to terrain system
        ...
        )
    {
        ...
        while(tire_mass - m_test > 0.0) {
            t += 1e-6;
            m_test =
                rho * (VolumeCyl(r_tire, r_tire - t_factor * t, width - 2 * t) + 2 * VolumeCyl(r_tire - t, r_rim, t));
        }
        ...
        return ChVector<>(
            Itip_tread + 2 * Itip_sidewall, Irot_tread + 2 * Irot_sidewall, Itip_tread + 2 * Itip_sidewall);
    }
