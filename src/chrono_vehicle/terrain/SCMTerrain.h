...
class CH_VEHICLE_API SCMLoader : public ChLoadContainer {
  public:
    ...
    std::chrono::duration<double, std::milli> m_duration_moving_patches = std::chrono::duration<double, std::milli>::zero();
    // std::chrono::duration<double, std::milli> m_duration_ray_testing = std::chrono::duration<double, std::milli>::zero();
    std::chrono::duration<double, std::milli> m_duration_ray_casting = std::chrono::duration<double, std::milli>::zero();
    std::chrono::duration<double, std::milli> m_duration_contact_patches = std::chrono::duration<double, std::milli>::zero();
  ...
  private:
    ...
    struct NodeRecord {
        ...
        int pre_contact_time;      //Shu:240325 add. If pre_contact_time is too long, reset the nr info.
        ...
        NodeRecord(double init_level, double level, const ChVector<>& n)
            : level_initial(init_level),
              ...
              erosion(false),
              massremainder(0),
              pre_contact_time(0), //Shu:240325 add
              step_plastic_flow(0) {}
    };
