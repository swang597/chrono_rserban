...
// Output
std::string out_dir = "../M113_PARALLEL";
...
static inline void TimingOutput(chrono::ChSystem* mSys,
    ...
    if (chrono::ChSystemParallel* parallel_sys = dynamic_cast<chrono::ChSystemParallel*>(mSys)) {
        auto msolver = std::static_pointer_cast<ChIterativeSolverParallel>(mSys->GetSolver());
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        dataFolderStream << "../M113_PARALLEL_" << r_g << "_" << rho_g << "_" << sphereRatio << "_" << mu_g << "_"
    ...
    statsFileStream << out_dir << "/M113_PARALLEL_" << r_g << "_" << rho_g << "_" << sphereRatio << "_" << mu_g << "_"
