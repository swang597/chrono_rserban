...
int main(int argc, char* argv[]) {
    ...
    vehicle.GetSystem()->Set_G_acc(ChVector<>(gravity * sin(alpha), 0, gravity * cos(alpha)));
    ...
    if (useDefSoil) {
        ...
        auto patch = terrain_R->AddPatch(material, ChVector<>(0, 0, terrainHeight), ChVector<>(0, 0, 1), terrainLength,
                                         terrainWidth);
    ...
    ChTrackedVehicleIrrApp app(&vehicle, L"M113 grade climbing");
    app.SetSkyBox();
    ...
    while (app.GetDevice()->run()) {
        ...
        // End simulation
        if ((time > tend) && (tend > 0) || vehicle.GetChassisBody()->GetPos().x() > 0.5 * terrainLength + rigidLength)
            break;
