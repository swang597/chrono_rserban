...
// output directory
const std::string out_dir = GetChronoOutputPath() + "GD250_RND_QUALITY";
...
int main(int argc, char* argv[]) {
    ...
    if(target_speed > 10) initLoc.x() -= 50;
    if(target_speed > 30) initLoc.x() -= 100;
    if(target_speed > 50) initLoc.x() -= 100;
    if(target_speed > 60) initLoc.x() -= 100;
    if(target_speed > 70) initLoc.x() -= 100;
