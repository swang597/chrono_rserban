...
std::string lugretire_testfile("hmmwv/tire/HMMWV_LugreTire.json");

// Output directories
std::string test_out_dir = "../TIRE_TEST_RIG/";
...
int main() {
    ...
    if (tire_model == TireModelType::ANCF || tire_model == TireModelType::REISSNER ||
        tire_model == TireModelType::FEA) {
    ...
    tex_rim->SetTextureFilename(GetChronoDataFile("pink.png"));
    ...
    switch (tire_model) {
        ...
        case TireModelType::LUGRE: {
            std::shared_ptr<ChLugreTire> tire_lugre;
            if (use_JSON) {
                tire_lugre = chrono_types::make_shared<LugreTire>(vehicle::GetDataFile(lugretire_testfile));
            }
            else {
                tire_lugre = chrono_types::make_shared<hmmwv::HMMWV_LugreTire>("Lugre tire");
            }
            tire_lugre->Initialize(rim, LEFT);
            tire_lugre->SetVisualizationType(VisualizationType::PRIMITIVES);
    ...
    std::string out_dir = test_out_dir + tire->GetTemplateName() + "/";

    if (!filesystem::create_directory(filesystem::path(test_out_dir))) {
        std::cout << "Error creating directory " << test_out_dir << std::endl;
        return 1;
    }
...
// my_system->DoStepDynamics(sim_step);
#ifdef USE_IRRLICHT
        // Render scene
        auto chassis_loc = chassis->GetPos();
        auto x = (irr::f32)chassis_loc.x();
        auto y = (irr::f32)chassis_loc.y();
        auto z = (irr::f32)chassis_loc.z();
        application->GetSceneManager()->getActiveCamera()->setPosition(core::vector3df(x, y + 1, z + 1));
        application->GetSceneManager()->getActiveCamera()->setTarget(core::vector3df(x, y, z));
        ...
        if (simTime >= outTime - sim_step / 2) {
            ...
            std::cout << "Time: " << simTime << std::endl
                      ...
                      << "Wheel Normal:             " << disc_normal.x() << ", " << disc_normal.y() << ", "
            ...
            out_tireforce << tireforceprint.point.x() << tireforceprint.point.y() << tireforceprint.point.z()
                          ...
                          << tireforceprint.moment.x() << tireforceprint.moment.y() << tireforceprint.moment.z()
                          << std::endl;
