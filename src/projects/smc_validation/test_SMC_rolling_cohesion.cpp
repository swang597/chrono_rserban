...
// =============================================================================
//
// This project simulates the collision of one sphere with a initial rotational
// velocity (around the collision axis) with a second stationary sphere. Use
...
void WriteData(std::ofstream& dat, ChSystemParallelSMC* msystem, const std::string& str) {
    for (int i = 0; i < msystem->Get_bodylist().size(); ++i) {
        ...
        double radius = 0;
        switch (
            pmodel->mData[0].type) {  // I'm assuming that there is only one object per collision model. Fix this later.
...
int main(int argc, char* argv[]) {
    ...
    const std::string projname = "test_rolling_cohesion";
    ...
    GetLog() << "\nTesting SMC Parallel rolling with cohesion behavior....\n";
    ...
    for (int f = 0; f < fmodels.size(); ++f) {
        ...
        while (time < time_sim) {
            ...
            if (KEthresh) {
                GetLog() << "[settling " << fmodels[f].c_str() << "] KE exceeds threshold at t = " << time << "\n";
                break;
            }
        }
        ...
        ChVector<> init_w = ChVector<>(0, 0, 1);
...
// Delete Visualization and check the results
        ...
        double wvel_diff = abs(body2->GetWvel_par().z() + body1->GetWvel_par().z());
    }
