...
int main(int argc, char* argv[]) {
    ...
    const std::string projname = "test_sliding_cohesion";
    ...
    GetLog() << "\nTesting SMC multicore sliding with cohesion behavior....\n";
    ...
    for (int f = 0; f < fmodels.size(); ++f) {
        ...
        // Write the initial state date and give th block a horizontal push
        ...
        // Check results. The error between d_thy and d_act should be >= 0.5 % ish,
        // and the block's velocity should be < 1e-4
        double d_thy = abs((bmass * init_bv.Length() * init_bv.Length()) / (2 * s_frict * ad));
        double d_act = body->GetPos().x() - bpos.x();
        double d_err = abs((d_thy - d_act) / d_thy) * 100;
        double vel_sphere = body->GetPos_dt().Length();

        GetLog() << "Error for " << fmodels[f].c_str() << " force model = " << d_err << " %\n";
    }
