...
void WVP_Vehicle::Create(bool fixed, SteeringType steering_model, ChassisCollisionType chassis_collision_type) {
    ...
    m_axles[0]->m_wheels[0] = chrono_types::make_shared<WVP_WheelLeft>("Wheel_FL");
    m_axles[0]->m_wheels[1] = chrono_types::make_shared<WVP_WheelRight>("Wheel_FR");
    m_axles[1]->m_wheels.resize(2);
    m_axles[1]->m_wheels[0] = chrono_types::make_shared<WVP_WheelLeft>("Wheel_RL");
    m_axles[1]->m_wheels[1] = chrono_types::make_shared<WVP_WheelRight>("Wheel_RR");
...
void WVP_Vehicle::Initialize(const ChCoordsys<>& chassisPos, double chassisFwdVel) {
    ...
    m_axles[1]->Initialize(m_chassis->GetBody(), ChVector<>(-4.039, 0, 0), ChVector<>(0), m_chassis->GetBody(), -1, 0.0,
                           m_omega[2], m_omega[3]);
