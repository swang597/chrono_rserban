...
double WVP_SpringForce::evaluate(double time, double rest_length, double length, double vel, const ChLinkTSDA& link) {
...
double WVP_ShockForce::evaluate(double time, double rest_length, double length, double vel, const ChLinkTSDA& link) {
    double length_mine;
    double length_other;
    double vel_mine;
    double vel_other;
...
double WVP_ShockForce::interpolate2D(double x, double y, std::map<double, std::shared_ptr<ChFunction_Recorder>> map2D) {
    ...
    std::map<double, std::shared_ptr<ChFunction_Recorder>>::iterator it = map2D.begin();
    double prevY = it->first;
	std::shared_ptr<ChFunction_Recorder> prevX = it->second;
    double currY = it->first;
	std::shared_ptr<ChFunction_Recorder> currX = it->second;
