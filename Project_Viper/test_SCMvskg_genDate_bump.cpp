...
int main(int argc, char* argv[]) {
    ...
    double terrain_hMin = 0, terrain_hMax = 0.15; //0.1; 
    ...
    bool flag_save_hmap_SCM = false; //true;
    ...
    double time_ramp_Wy = 0, Wy = 50 * CH_C_RPM_TO_RPS;
    ...
    std::string output_folderpath = out_dir + "_HMbmp_terrain_hMax"+std::to_string(terrain_hMax)+
        "_initX"+std::to_string(terrain_initX)+
    ...
    for (int istep = 0; istep < num_steps; istep++) {
        ...
        if (flag_save_hmap_SCM && (remainder < 1e-6 || (dt_dump - remainder) < 1e-6)) {
            // save heightmap 
