    ...
    virtual void Setup() override {
        ...
        for (int iwheel = 0; iwheel < 4; iwheel++) {
            ...
            if(ChTime < m_SCM_ML_switch){
                ...
                // std::cout << "inI_1chan[0][0][0][0]" << inI_1chan[0][0][0][0] << std::endl;
            ...
            inI_1chan_nm = (positions[2] - inI_1chan - m_I_min_max[0]) / (m_I_min_max[1] - m_I_min_max[0]);
            // write_output(m_output_folderpath + "NN_Iin_Tat" + std::to_string(ChTime) + ".txt", inI_1chan[0][0]);
            // if(inI_1chan_nm.min().item() < 0.0 || inI_1chan_nm.max().item() > 1.0){
            //     std::cout << "inI_1chan_nm.min()=" << inI_1chan_nm.min().item() << ", inI_1chan_nm.max()=" << inI_1chan_nm.max().item() << std::endl;
            // }
            ...
            inV_ts = torch::tensor({
                m_wheels[iwheel]->GetPos_dt().x(), m_wheels[iwheel]->GetPos_dt().z(), m_wheels[iwheel]->GetWvel_par().y()
                }).to(torch::kFloat32);
...
int main(int argc, char* argv[]) {
    ...
    std::string folderpath_normlized = "/home/swang597/Documents/Research/chrono_fork_rserban/Project_Viper/build_SCM_genData/DEMO_OUTPUT/Dataset_train_96by72_2phase_varVx_relI00V0_F0_240430/";
    
    std::string model_path = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/Model_2stepNN_img96by72_varKsize3333_ViperCylindWheel_I00V0_F0_240430/";
