...
void applyWheelSinkage(torch::Tensor& heightMap, torch::Tensor wheelCenter, float radius, float width, float gridSize) {
    ...
    auto cx = (nx * 0.5) * gridSize;
    auto cy = (ny * 0.5) * gridSize;
...
class TerrainForceLoaderML : public ChLoadContainer {
  public:
    TerrainForceLoaderML(const std::string& input_folderpath, std::string& output_folderpath, 
        ...
        TorchModelRunner model_runner_Fx, TorchModelRunner model_runner_Fz,TorchModelRunner model_runner_Ty,
        ...
        m_model_runner_Fx(model_runner_Fx),m_model_runner_Fz(model_runner_Fz),m_model_runner_Ty(model_runner_Ty),
        ...
        m_duration_Img(0), m_duration_F(0), m_duration_all(0), m_SCM_ML_switch(SCM_ML_switch){
    ...
    virtual void Setup() override {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            positions = {static_cast<float>(m_wheels[i]->GetPos().x() + m_terrain_initLoc[0]), 
                         static_cast<float>(m_wheels[i]->GetPos().y() + m_terrain_initLoc[1]), 
                         static_cast<float>(m_wheels[i]->GetPos().z() - m_terrain_initLoc[2])};
            ...
            if(ChTime < m_SCM_ML_switch){
                filename_hmap_I00 = m_output_folderpath +  "hmap_SCM_wheel" + std::to_string(i) + "_Tat"+ std::to_string(ChTime) +".txt";
                ...
                inI_1chan = m_wheels[i]->GetPos().z() - inI_1chan;
            ...
            // inV_ts = torch::tensor({m_wheels[i]->GetPos().z() - m_terrain_initLoc[2],
            //     m_wheels[i]->GetPos_dt().x(), m_wheels[i]->GetPos_dt().z(),
            //     }).to(torch::kFloat32);
            inV_ts = torch::tensor({m_wheels[i]->GetPos_dt().x(), m_wheels[i]->GetPos_dt().z(),
                }).to(torch::kFloat32);
            ...
            for (int i = 0; i < m_Vec_min_max.size(1); i++){
                ...
                if(diff.item<double>() < 1e-6){
                    ...
                    // std::cout << "inV_ts_nm["<<i<<"]=" << inV_ts_nm[i].item<double>() << << std::endl;
                } 
            }     
            ...
            // std::cout << "inV_ts=" << inV_ts[0].item()<<","<< inV_ts[1].item()<<","<< inV_ts[2].item() << std::endl;
            // std::cout << "inV_ts_nm=" << inV_ts_nm[0].item()<<","<< inV_ts_nm[1].item()<<","<< inV_ts_nm[2].item() << std::endl;
            // std::cout << "Vec_min="<<m_Vec_min_max[0][0].item()<<","<<m_Vec_min_max[0][1].item()<<","<<m_Vec_min_max[0][2].item()<< std::endl;
            // std::cout << "Vec_max="<<m_Vec_min_max[1][0].item()<<","<<m_Vec_min_max[1][1].item()<<","<<m_Vec_min_max[1][2].item()<< std::endl;
            // std::cout << "Vx_min_max="<<m_Vec_min_max[0][1].item()<<","<<m_Vec_min_max[1][1].item()<< std::endl;
            // std::cout << "Vz_min_max="<<m_Vec_min_max[0][2].item()<<","<<m_Vec_min_max[1][2].item()<< std::endl;

            inV_ts_nm = inV_ts_nm.unsqueeze(0);
            // std::cout << "line 250" << std::endl;
            auto start_F = std::chrono::high_resolution_clock::now();
            at::Tensor Fx_ts_nm = m_model_runner_Fx.runModel(inI_1chan_nm, inV_ts_nm);
            at::Tensor Fz_ts_nm = m_model_runner_Fz.runModel(inI_1chan_nm, inV_ts_nm);
            at::Tensor Ty_ts_nm = m_model_runner_Ty.runModel(inI_1chan_nm, inV_ts_nm);
            auto end_F = std::chrono::high_resolution_clock::now();
            at::Tensor F_ts_nm = torch::cat({Fx_ts_nm, Fz_ts_nm, Ty_ts_nm}, 1);
            ...
            // std::cout << "inI_1chan.size" << inI_1chan.sizes() << std::endl;
            // if(m_crt_frame % 20 == 0){
            double remainder = std::fmod(ChTime, 0.01);
  ...
  private:
    ...
    TorchModelRunner m_model_runner_Fx,m_model_runner_Fz,m_model_runner_Ty;
    Heightmap m_HM;
...
int main(int argc, char* argv[]) {
    ...
    std::string output_folderpath = out_dir + "_HMflat_SCM2MLSwitch" + std::to_string(SCM_ML_switch) + "_dt" + std::to_string(dt) + "_240411/";
    ...
    std::string model_path_Fx = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/Model_2stepNN_iDT1_img96by72_varKsize3333_varVx_relI00V0_Fx_240416/";
    std::string model_path_Fz = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/Model_2stepNN_iDT1_img96by72_varKsize3333_varVx_relI00V0_Fz_240416/";
    std::string model_path_Ty = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/Model_2stepNN_iDT1_img96by72_varKsize3333_varVx_relI00V0_Ty_240416/";
    // std::string model_path_Img;
    std::string model_path_Fx, model_path_Fz, model_path_Ty;
    // model_path_Img = model_path_Img0 + "modelImg_cpu.pt";
    model_path_Fx =   model_path_Fx + "modelF_cpu.pt";
    model_path_Fz =   model_path_Fz + "modelF_cpu.pt";
    model_path_Ty =   model_path_Ty + "modelF_cpu.pt";
    // TorchModelRunner model_runner_Img(model_path_Img);
    TorchModelRunner model_runner_Fx(model_path_Fx);
    TorchModelRunner model_runner_Fz(model_path_Fz);
    TorchModelRunner model_runner_Ty(model_path_Ty);
    ...
    auto terrain_ML = chrono_types::make_shared<TerrainForceLoaderML>(folderpath_normlized,
                        output_folderpath, wheels, model_runner_Fx,model_runner_Fy,model_runner_Ty, HM, 
                        terrain_initLoc,
    ...
    for (int istep = 0; istep < num_steps; istep++) {
        ...
        // if(sys.GetChTime() < SCM_ML_switch){
            // save heightmap 
            for(int i = 0; i < 4; i++) {
                ChVector<> wheel_pos = wheels[i]->GetPos();
                HMfilename_SCM = output_folderpath + "hmap_SCM_wheel" + std::to_string(i) + "_Tat" + std::to_string(istep * dt) + ".txt";
        ...
        // if ((istep+1) % 20 == 0){
        double remainder = std::fmod(time, 0.01);
        if (remainder < 1e-6 || (0.01 - remainder) < 1e-6) {
            SCM_forces << time << "    ";
            for (int i = 0; i < 4; i++) {
