...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::shared_ptr<ChShaft>> driveshafts{
        viper->GetDriveshaft(ViperWheelID::V_LF),  //
        viper->GetDriveshaft(ViperWheelID::V_RF),  //
        viper->GetDriveshaft(ViperWheelID::V_LB),  //
        viper->GetDriveshaft(ViperWheelID::V_RB)   //
    };
    ...
    std::to_string(SCM_ML_switch) + "_dt" + std::to_string(dt) + "_240422/";
    ...
    std::ofstream Wheel_states(output_folderpath + "Wheel_states.txt", std::ios::trunc);
    ...
    for (int istep = 0; istep < num_steps; istep++) {
        ...
        // Save vehicle states
            ...
            Wheel_states << time << "   ";
            for (int i = 0; i < 4; i++) {
                // Wheel_states << wheels[i]->GetRot() << "   " << wheels[i]->GetWvel_par()
                //             << "   " << wheels[i]->GetRot().RotateBack(wheels[i]->GetWvel_par())
                //             << "   ";
                Wheel_states << driveshafts[i]->GetPos_dt() 
                            << "   ";
            }
            Wheel_states << endl;
        }
