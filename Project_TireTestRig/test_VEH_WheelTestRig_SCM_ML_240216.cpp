...
class TerrainForceLoader : public ChLoadContainer {
  public:
    ...
    std::shared_ptr<hmmwv::HMMWV_Wheel> wheel, 
        ...
        m_dI_min_max = loadFromTxt(fn_dataPT + "dI_min_max_p1.txt");
        m_F_min_max = loadFromTxt(fn_dataPT + "F_min_max_p1.txt");
        m_dF_min_max = loadFromTxt(fn_dataPT + "dF_min_max_p1.txt");
        ...
        std::cout << "m_dI_min_max=" << m_dI_min_max << std::endl;
        std::cout << "m_F_min_max=" << m_F_min_max << std::endl;
        std::cout << "m_dF_min_max=" << m_dF_min_max << std::endl;
    ...
    // // NN_I & NN_F, use SCM's heightmap, which is saved as txt files
    // Apply forces to wheel bodies, at the beginning of each timestep.
    virtual void Setup() override {
        // std::cout << "line 175: Setup() m_crt_frame="<< m_crt_frame << std::endl;
        GetLoadList().clear();
        // std::cout << "line177:" << std::endl;
        ...
        // std::string folder_HM_SCM = "/home/swang597/Documents/Research/chrono_fork_rserban/Project_TireTestRig/build_SCM_ML/DEMO_OUTPUT/SCM_ML_TIRE_TEST_RIG_NNI_NNF_SCMHM_dt0.000500_terrGrid0.005000terrX-23.000000terrH-1.000000normLoad1000.000000/";
        ...
        // if(m_crt_frame % 10 == 0){
            ...
            inV_ts = torch::tensor({m_wheel_state.pos[2] - m_terrain_initLoc[2],
                    m_wheel_state.lin_vel[0], m_wheel_state.lin_vel[2], m_wheel_state.ang_vel[1]
                    }).to(torch::kFloat32);
            ...
            // std::cout << "inV_ts_nm.sizes()=" << inV_ts_nm.sizes() << std::endl;

            // run models
            // std::cout << "****** inI_1chan_nm.sizes()=" << inI_1chan_nm.sizes() << std::endl;
            // std::cout << "****** inV_ts_nm.sizes()=" << inV_ts_nm.sizes() << ",inV_ts_nm=" <<inV_ts_nm << std::endl;
            at::Tensor dI_nm = m_model_runner_Img.runModel(inI_1chan_nm, inV_ts_nm);
            ...
            dI = dI_nm * (m_dI_min_max[1] - m_dI_min_max[0]) + m_dI_min_max[0];
            ...
            at::Tensor F_ts_nm = m_model_runner_F.runModel(inI_2chan_nm, inV_ts_nm);
            // if F_ts_nm larger than 1, set it to 1; if smaller than 0, set it to 0
            // F_ts_nm = torch::clamp(F_ts_nm, 0.0, 1.0);
        ...
        // if(m_wheel_state.pos[0] > 1.09){
            // fix Vx & Wy, when t=1.5, x=1.097 
        if(m_wheel_state.pos[0] > 0.109){
            ...
            if(inV_ts_nm[0][2].item<double>() < 0.0 || inV_ts_nm[0][2].item<double>() > 1.0){
                F_ts[0][1] = F_ts[0][1] * 2;
                std::cout << "Vz is out of range, need to enlarge the Fz" << std::endl;
            }
    ...
    // only use NN_F, all other information comes from SCM
    // // Apply forces to wheel bodies, at the beginning of each timestep.
    ...
    //     F_ts = torch::zeros({1,3}).to(torch::kFloat32);
    //     // std::cout << "line 189" << std::endl;
    //     std::string folder_HM_SCM = "/home/swang597/Documents/Research/chrono_fork_rserban/Project_TireTestRig/build_SCM_ML/DEMO_OUTPUT/SCM_ML_TIRE_TEST_RIG_dt0.000500_terrGrid0.005000terrX-23.000000terrH-1.000000normLoad1000.000000/";
    //     std::string filename_hmap_I11, filename_hmap_I10;
    //     // std::cout << "line 192" << std::endl;
    //     if(m_crt_frame % 10 == 0 and m_crt_frame > 0){
    //         filename_hmap_I10 = folder_HM_SCM + "hmap_Pat" + std::to_string(ChTime - 0.005) + "_Tat"+ std::to_string(ChTime) +".txt";
    //         filename_hmap_I11 = folder_HM_SCM + "hmap_Pat" + std::to_string(ChTime - 0.005) + "_Tat"+ std::to_string(ChTime - 0.005) +".txt";
    //         std::cout << "line 196" << std::endl;
    //         if(!std::filesystem::exists(filename_hmap_I11)){
    //             std::cout << "File " << filename_hmap_I11 << " does not exist." << std::endl;
    //             exit(1);
    //         }
    //         if(!std::filesystem::exists(filename_hmap_I10)){
    //             std::cout << "File " << filename_hmap_I10 << " does not exist." << std::endl;
    //             exit(1);
    //         }
    //         std::cout << "line 205" << std::endl;
    //         inI_1chan = loadFromTxt(filename_hmap_I11);
    //         std::cout << "line 207" << std::endl;
    //         dI = loadFromTxt(filename_hmap_I10); // hmap_I10
    //         std::cout << "line 209" << std::endl;
    //         dI = inI_1chan - dI; // dI = hmap_I11 - hmap_I10
    //         std::cout << "line 211" << std::endl;
    //         inI_1chan = inI_1chan - m_terrain_initLoc[2];
    //         std::cout << "max(inI_1chan)=" << inI_1chan.max().item() << ", min(inI_1chan)=" << inI_1chan.min().item() << std::endl;
    //         std::cout << "max(dI)=" << dI.max().item() << ", min(dI)=" << dI.min().item() << std::endl;
    //         // std::cout << "inI_1chan.sizes()=" << inI_1chan.sizes() << ", dI.sizes()=" << dI.sizes() << std::endl;
    //         std::cout << "line 208" << std::endl;
            
    //         inI_1chan_nm = (inI_1chan - m_I_min_max[0]) / (m_I_min_max[1] - m_I_min_max[0]);
    //         // inI_1chan_nm = torch::clamp(inI_1chan_nm, 0.0, 1.0);
    //         dI_nm = (dI - m_dI_min_max[0]) / (m_dI_min_max[1] - m_dI_min_max[0]);
    //         // std::cout << "line 213" << std::endl;
    //         // std::cout << "inI_1chan_nm.sizes()=" << inI_1chan_nm.sizes() << ", dI_nm.sizes()=" << dI_nm.sizes() << std::endl;
    //         std::cout << "max(dI_nm)=" << dI_nm.max().item() << ", min(dI_nm)=" << dI_nm.min().item() << std::endl;
    //         std::cout << "max(inI_1chan_nm)=" << inI_1chan_nm.max().item() << ", min(inI_1chan_nm)=" << inI_1chan_nm.min().item() << std::endl;
            
    //         inI_1chan_nm = inI_1chan_nm.unsqueeze(0).unsqueeze(0);
    //         dI_nm = dI_nm.unsqueeze(0).unsqueeze(0);
    //         // std::cout << "inI_1chan_nm.sizes()=" << inI_1chan_nm.sizes() << ", dI_nm.sizes()=" << dI_nm.sizes() << std::endl;
            
    //         // std::cout << "line 216" << std::endl;
    //         inI_2chan_nm = torch::cat({inI_1chan_nm, dI_nm}, 1);
    //         // std::cout << "inI_2chan_nm.sizes()=" << inI_2chan_nm.sizes() << std::endl;
    //         inV_ts = torch::tensor({m_wheel_state.pos[2] - m_terrain_initLoc[2],
    //                 m_wheel_state.lin_vel[0], m_wheel_state.lin_vel[2], m_wheel_state.ang_vel[1]
    ...
    //             // std::cout << "m_Vec_min_max["<< i<<"] = " << m_Vec_min_max[i][0].item() << ","<< m_Vec_min_max[i][1].item()  << std::endl;
    //         }            
    //         // std::cout << "max(inV_ts_nm)=" << inV_ts_nm.max() << ", min(inV_ts_nm)=" << inV_ts_nm.min() << std::endl;
    //         // inV_ts_nm = torch::clamp(inV_ts_nm, 0.0, 1.0);
    //         std::cout << "inV_ts=" << inV_ts << ",inV_ts_nm=" << inV_ts_nm<< std::endl;
    //         inV_ts_nm = inV_ts_nm.unsqueeze(0);
    //         // std::cout << "inV_ts_nm.sizes()=" << inV_ts_nm.sizes() << std::endl;
    //         // std::cout << "line 225" << std::endl;
    //         // write_output(m_output_folderpath + "I_t" + std::to_string(ChTime) + ".txt", position_tensor.unsqueeze(0));
    //         // write_output(m_output_folderpath + "I_t" + std::to_string(ChTime) + ".txt", inI_1chan[0][0], true);
            
    //         // write_output(m_output_folderpath + "Inm_t" + std::to_string(ChTime) + ".txt", position_tensor.unsqueeze(0));
    //         // write_output(m_output_folderpath + "Inm_t" + std::to_string(ChTime) + ".txt", inI_1chan_nm[0][0], true);
            
    //         // // std::cout << "write I done "<< std::endl;
    //         // write_output(m_output_folderpath + "dI_t" + std::to_string(ChTime) + ".txt", dI[0][0]);
    //         // write_output(m_output_folderpath + "dInm_t" + std::to_string(ChTime) + ".txt", dI_nm[0][0]);
    ...
    //         // F_ts[1] -= damping * m_wheel->GetLinVel().z();
    //         // F_ts[1] -= 100 * m_wheel->GetLinVel().z();
    ...
    // // NN_I & NN_F using ML's Heightmap
    // // Apply forces to wheel bodies, at the beginning of each timestep.
    // virtual void Setup() override {
    //     // Reset load list
    //     // std::cout << "line 175: Setup()" << std::endl;
    //     GetLoadList().clear();
    //     // std::cout << "line177:" << std::endl;
    //     // Generate loads for this time step
    //     torch::Tensor inI_1chan, inV_ts, F_ts, dI; 
    ...
    //     F_ts = torch::zeros({1,3}).to(torch::kFloat32);
    //     // std::cout << "line 191 m_wheel_stat  e.pos[2]=" << m_wheel_state.pos[2] << std::endl;
    //     // if(m_wheel_state.pos[2] < -0.05){
    //         // std::cout << "m_terrian_initLoc=" << m_terrain_initLoc << std::endl;
    ...
    //         inI_1chan_nm = (inI_1chan - m_I_min_max[0]) / (m_I_min_max[1] - m_I_min_max[0]);
    //         // if inI_1chan_nm larger than 1, set it to 1; if smaller than 0, set it to 0
    //         inI_1chan_nm = torch::clamp(inI_1chan_nm, 0.0, 1.0);
    ...
    //         // How to calculate z? z = wheel_pos_z - terrain_initH. In prepare_data, set -1 as datum height. <not a good datum>
    //         inV_ts = torch::tensor({m_wheel_state.pos[2] - m_terrain_initLoc[2],
    //             m_wheel_state.lin_vel[0], m_wheel_state.lin_vel[2], m_wheel_state.ang_vel[1]
    //             }).to(torch::kFloat32);
    ...
    //         // std::cout << "m_Vec_min_max[0]:" << m_Vec_min_max[0] << std::endl;
    //         // std::cout << "m_Vec_min_max[1]:" << m_Vec_min_max[1] << std::endl;
    //         // std::cout << "m_F_min_max[0]:" << m_F_min_max[0] << std::endl;
    //         // std::cout << "m_F_min_max[1]:" << m_F_min_max[1] << std::endl;
    ...
    //         double print_interval = 0.1; // Interval at which to print
    ...
    //             std::cout << "write F at time " << ChTime <<", m_crt_frame= "<< m_crt_frame<< std::endl;
    //             write_output(m_output_folderpath + "F_t" + std::to_string(ChTime) + ".txt", F_ts);
    //             write_output(m_output_folderpath + "Fnm_t" + std::to_string(ChTime) + ".txt", F_ts_nm);
    //         }
    ...
    //         // std::cout << "m_wheel_state.pos=" << m_wheel_state.pos << std::endl;
    //     // }
    //     auto force_load = chrono_types::make_shared<ChLoadBodyForce>(m_wheel->GetSpindle(), ChVector<>(F_ts[0][0].item<double>(), 0.0, F_ts[0][1].item<double>()), false,
    //                                                                     m_wheel_state.pos, false);
    //     auto torque_load =
...
void SaveHeightmap(ChTerrain* terrain, double pos_x, double pos_y, double heightmap_grid, 
    ...
    std::string out_dir, int istep_pos, int istep_cur, double dt, int nx = 96, int ny = 72){
        ...
        // std::cout << "HMfilename=" << HMfilename << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    std::string folderpath_normlized = "/home/swang597/Documents/Research/chrono_fork_radu/project_TireTestRig/build/DEMO_OUTPUT/Dataset_4_ML_train_96by72_2phase_240212/";
    std::string model_path = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/Model_py2cpp_2stepNN_iDT1_img96by72_varKsize3333_largerHM_2phase_240216/";
    std::string model_path_Img;
    std::string model_path_F;
    model_path_Img = model_path + "modelImg_cpu.pt";
    ...
    // rig.SetLongSpeedFunction(chrono_types::make_shared<ChFunction_Const>(2)); // fix Vx
    rig.SetAngSpeedFunction(chrono_types::make_shared<ChFunction_Const>(10 * CH_C_RPM_TO_RPS)); // fix omega
...
// Create the Blender exporter
    ...
    for (int istep = 0; istep < num_steps; istep++) {
        ...
        // if flag_save_figure and istep % freq_save_figure == 0:
        //  imgName = foldername + 'img_' + time_str + '.jpg'
