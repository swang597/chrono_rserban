...
class TerrainForceLoader : public ChLoadContainer {
  public:
    ...
    std::shared_ptr<hmmwv::HMMWV_Wheel> wheel, 
        torch::jit::script::Module module_AutoEC, torch::jit::script::Module module_F, 
        ...
        m_module_AutoEC(module_AutoEC), m_module_F(module_F), 
    ...
    virtual void Setup() override {
        ...
        std::string folder_HM_SCM = "/home/swang597/Documents/Research/chrono_fork_rserban/Project_TireTestRig/build_SCM_ML_HMreduc/DEMO_OUTPUT/SCM_ML_HMReduc_TIRE_TEST_RIG_dt0.000500_terrGrid0.005000terrX-23.000000terrH-1.000000normLoad1000.000000/";
        ...
        if(m_crt_frame % 10 == 0){
            ...
            // if(inI_1chan.sizes()[0] != 96){
            //     std::cout << "inI_1chan.sizes()=" << inI_1chan.sizes() << std::endl;
            //     inI_1chan = inI_1chan[0:96][:];
            // }
            // if(inI_1chan.sizes()[1] != 72){
            //     std::cout << "inI_1chan.sizes()=" << inI_1chan.sizes() << std::endl;
            //     inI_1chan = inI_1chan[:][0:96];
            // }
            ...
            // std::cout << "line 265" << std::endl;
            write_output(m_output_folderpath + "Vec_t" + std::to_string(ChTime) + ".txt", inV_ts.unsqueeze(0));
            // std::cout << "line 267" << std::endl;
            write_output(m_output_folderpath + "Vecnm_t" + std::to_string(ChTime) + ".txt", inV_ts_nm.unsqueeze(0));
            // std::cout << "line 269" << std::endl;
            // std::cout << "inI_2chan_nm.sizes()=" << inI_2chan_nm.sizes() << std::endl;
            // std::cout << "inV_ts_nm.sizes()=" << inV_ts_nm.sizes() << std::endl;
            std::vector<torch::jit::IValue> inputs_HM_reduced;
            // std::vector<c10::IValue> inputs_HM_reduced;
            // std::cout << "line 271" << std::endl;
            inI_1chan_nm = inI_1chan_nm.view({1,-1});
            inV_ts_nm = inV_ts_nm.unsqueeze(0);
            inputs_HM_reduced.push_back(inI_1chan_nm); // Wrap the tensor in a IValue before passing
            std::cout << "inI_1chan_nm.sizes()=" << inI_1chan_nm.sizes() << std::endl;
            // std::cout << "line 272" << std::endl;
            // std::cout << "inputs_HM_reduced=" << inputs_HM_reduced << std::endl;
            // std::cout << "inputs_HM_reduced.size()=" << inputs_HM_reduced.size() << std::endl;
            // std::cout << "inputs_HM_reduced[0].size()=" << inputs_HM_reduced[0].size() << std::endl;
            // at::Tensor HM_decode = m_module_AutoEC.forward(inputs_HM_reduced).toTensor();
            // std::cout << "HM_decode.sizes()=" << HM_decode.sizes() << std::endl;
            auto HM_reduced = m_module_AutoEC.get_method("encode")(inputs_HM_reduced).toTensor();
            // std::cout << "line 275" << std::endl;
            // std::cout << "HM_reduced.sizes()=" << HM_reduced.sizes() << std::endl;
            // std::cout << "inV_ts_nm.sizes()=" << inV_ts_nm.sizes() << std::endl;
            torch::Tensor input_HMred_Vec = torch::cat({HM_reduced, inV_ts_nm}, 1);
            // std::cout << "line 276" << std::endl;
            
            std::vector<torch::jit::IValue> inputs_HMred_Vec;
            inputs_HMred_Vec.push_back(input_HMred_Vec); // Wrap the tensor in a IValue before passing
            // std::cout << "line 280" << std::endl;
            at::Tensor F_ts_nm = m_module_F.forward(inputs_HMred_Vec).toTensor();
            // std::cout << "line 282" << std::endl;

            // at::Tensor F_ts_nm = m_module_F.runModel(inI_2chan_nm, inV_ts_nm);
  ...
  private:
    ...
    torch::jit::script::Module m_module_AutoEC, m_module_F;
    Heightmap m_HM;
...
int main(int argc, char *argv[]) {
    ...
    // Output directory
    const std::string out_dir = GetChronoOutputPath() + "SCM_ML_HMReduc_TIRE_TEST_RIG_dt" + std::to_string(dt) + "_terrGrid" +
    ...
    std::string model_path = "/home/swang597/Documents/Research/Project_heightmap/Code/Pytorch_cpp_model/Model/";
    std::string model_path_F = model_path + "Model_py2cpp_2phase_HMReduced_240220/model_HM_reduced_p1_cpp_cpu.pth";
    std::string model_path_AutoEC = model_path + "Autoencoder/autoencoder_best_model_p1_cpp_cpu.pth";
    // TorchModelRunner model_runner_Img(model_path_Img);
    // TorchModelRunner model_runner_F(model_path_F);
    torch::jit::script::Module module_AutoEC, module_F;
    try {
        // Deserialize the ScriptModule from a file using torch::jit::load().
        module_AutoEC = torch::jit::load(model_path_AutoEC);
        module_F = torch::jit::load(model_path_F);
    }
    ...
    auto terrain_ML = chrono_types::make_shared<TerrainForceLoader>(folderpath_normlized,
                        output_folderpath, wheel, module_AutoEC, module_F, HM, terrain_initLoc);
