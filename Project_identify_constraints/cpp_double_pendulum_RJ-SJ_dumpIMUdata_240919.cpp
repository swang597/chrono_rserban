...
int main(int argc, char* argv[]) {
    ...
    pendulumBody1->SetPos_dt(ChVector<>(5, 0, 0));
    ...
    // ChVector<> rotation_axis(0, 0, 1);
    // rotation_axis.Normalize(); // Ensure the axis is a unit vector
    // auto motor = std::make_shared<ChLinkMotorRotationSpeed>();
    // motor->Initialize(floorBody, pendulumBody1, ChFrame<>(ChVector<>(0, 4, 0), Q_from_AngAxis(0, rotation_axis))); // Rotate around the arbitrary axis
    // motor->SetSpeedFunction(chrono_types::make_shared<ChFunction_Const>(CH_C_PI / 2)); // Constant speed of 0.1 * 90 degrees per second
    // sys.AddLink(motor);


    // // 3 - Define the rotation axis along the z-axis (0, 0, 1)
    // ChVector<> rotation_axis(0, 0, 1);
    // // Create the motor and initialize it with a z-axis rotation
    // auto motor = std::make_shared<ChLinkMotorRotationSpeed>();
    // motor->Initialize(floorBody, pendulumBody1, ChFrame<>(ChVector<>(0, 4, 0), Q_from_AngAxis(0, rotation_axis))); // Rotate around the z-axis

    // // Create a sine function for the angular velocity: amplitude = CH_C_PI / 2, frequency = 1 Hz, phase = 0
    // auto sine_function = chrono_types::make_shared<ChFunction_Sine>(0, 0.1, CH_C_PI / 2);  // ChFunction_Sine(phase, frequency, amplitude)

    // // Set the sine function as the speed function for the motor
    // motor->SetSpeedFunction(sine_function);
    ...
    // 4 - Create a revolute joint
    auto revoluteLink1 = std::make_shared<ChLinkLockRevolute>();  // Revolute joint constrains x, y, z, and allows rotation about a single axis (usually z)

    // Define the position of the revolute joint in absolute coordinates
    ChFrame<> link_position_abs1(ChVector<>(0, 4, 0));

    // Initialize the revolute joint (rotation around z-axis)
    // revoluteLink1->Initialize(pendulumBody1,        // the 1st body to connect
    //                         floorBody,            // the 2nd body to connect
    //                         ChCoordsys<>(ChVector<>(0, 4, 0),   // Joint position
    ...
    double body2_vz = 2.0;
    pendulumBody2->SetPos_dt(ChVector<>(0.0, 0.0, body2_vz));  // Velocity in m/s along x-axis
    ...
    double num_steps = int(100 / step_size);
    const std::string out_dir = GetChronoOutputPath() 
        + "Double_pendulum_RJ_SJ_3D_RJaxis111_dt" +std::to_string(step_size) 
        + "_v" + std::to_string(body2_vz)+ "/";
    // Initialize output
